import React from 'react';

type Props = {};

function Todo({}: Props) {
  return (
    <>
      <section className="border border-[#E5E5E5] bg-[#F9F9F9] p-4 shadow-xl mb-5 rounded-md">
        <div className="flex justify-between items-center">
          <h3 className="font-poppins font-medium mb-5">Todo</h3>
          <button className="font-poppins font-medium mb-5 underline underline-offset-4">
            Add Todo
          </button>
        </div>
        <div className="flex flex-col gap-5">
          <div className="border border-[#666666] p-3 pb-16 rounded-md bg-white relative">
            <div className="flex items-center gap-1">
              <p className="font-poppins font-light text-[#221C1D] bg-[#B8EBB0] px-2 py-1 rounded-full mb-2 inline-block">
                Low
              </p>
              <p className="font-poppins text-base font-semibold text-[#221C1D] py-1 px-2 mb-2">
                Lorem Ipsum
              </p>
            </div>
            <p className="font-poppins text-base font-medium text-[#221C1D] py-1">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
            </p>
            <div className="flex gap-3 py-1">
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#DE1D6E] rounded-md">
                Delete
              </button>
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#4F1DDE] rounded-md">
                Edit
              </button>
            </div>
            <p className="font-poppins font-light text-sm pt-2 text-[#666666]">
              Mar 3, 2020
            </p>
            <div className="absolute bottom-0 right-0 mb-2 mr-3 inline-flex items-center justify-center w-12 h-12 overflow-hidden bg-gray-100 rounded-full">
              <span className="font-medium text-gray-600">A</span>
            </div>
          </div>

          <div className="border border-[#666666] p-3 pb-16 rounded-md bg-white relative">
            <div className="flex items-center gap-1">
              <p className="font-poppins font-light text-[#221C1D] bg-[#F0CA81] px-2 py-1 rounded-full mb-2 inline-block">
                Medium
              </p>
              <p className="font-poppins text-base font-semibold text-[#221C1D] py-1 px-2 mb-2">
                Lorem Ipsum
              </p>
            </div>
            <p className="font-poppins text-base font-medium text-[#221C1D] py-1">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
            </p>
            <div className="flex gap-3 py-1">
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#DE1D6E] rounded-md">
                Delete
              </button>
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#4F1DDE] rounded-md">
                Edit
              </button>
            </div>
            <p className="font-poppins font-light text-sm pt-2 text-[#666666]">
              Mar 3, 2020
            </p>
            <div className="absolute bottom-0 right-0 mb-2 mr-3 inline-flex items-center justify-center w-12 h-12 overflow-hidden bg-gray-100 rounded-full">
              <span className="font-medium text-gray-600">A</span>
            </div>
          </div>

          <div className="border border-[#666666] p-3 pb-16 rounded-md bg-white relative">
            <div className="flex items-center gap-1">
              <p className="font-poppins font-light text-white bg-[#DE1D3E] px-2 py-1 rounded-full mb-2 inline-block">
                High
              </p>
              <p className="font-poppins text-base font-semibold text-[#221C1D] py-1 px-2 mb-2">
                Lorem Ipsum
              </p>
            </div>
            <p className="font-poppins text-base font-medium text-[#221C1D] py-1">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
            </p>
            <div className="flex gap-3 py-1">
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#DE1D6E] rounded-md">
                Delete
              </button>
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#4F1DDE] rounded-md">
                Edit
              </button>
            </div>
            <p className="font-poppins font-light text-sm pt-2 text-[#666666]">
              Mar 3, 2020
            </p>
            <div className="absolute bottom-0 right-0 mb-2 mr-3 inline-flex items-center justify-center w-12 h-12 overflow-hidden bg-gray-100 rounded-full">
              <span className="font-medium text-gray-600">A</span>
            </div>
          </div>

          <div className="border border-[#666666] p-3 pb-16 rounded-md bg-white relative">
            <div className="flex items-center gap-1">
              <p className="font-poppins font-light text-[#221C1D] bg-[#B8EBB0] px-2 py-1 rounded-full mb-2 inline-block">
                Low
              </p>
              <p className="font-poppins text-base font-semibold text-[#221C1D] py-1 px-2 mb-2">
                Lorem Ipsum
              </p>
            </div>
            <p className="font-poppins text-base font-medium text-[#221C1D] py-1">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
            </p>
            <div className="flex gap-3 py-1">
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#DE1D6E] rounded-md">
                Delete
              </button>
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#4F1DDE] rounded-md">
                Edit
              </button>
            </div>
            <p className="font-poppins font-light text-sm pt-2 text-[#666666]">
              Mar 3, 2020
            </p>
            <div className="absolute bottom-0 right-0 mb-2 mr-3 inline-flex items-center justify-center w-12 h-12 overflow-hidden bg-gray-100 rounded-full">
              <span className="font-medium text-gray-600">A</span>
            </div>
          </div>

          <div className="border border-[#666666] p-3 pb-16 rounded-md bg-white relative">
            <div className="flex items-center gap-1">
              <p className="font-poppins font-light text-[#221C1D] bg-[#B8EBB0] px-2 py-1 rounded-full mb-2 inline-block">
                Low
              </p>
              <p className="font-poppins text-base font-semibold text-[#221C1D] py-1 px-2 mb-2">
                Lorem Ipsum
              </p>
            </div>
            <p className="font-poppins text-base font-medium text-[#221C1D] py-1">
              Lorem ipsum dolor sit amet, consectetur adipisicing elit.
            </p>
            <div className="flex gap-3 py-1">
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#DE1D6E] rounded-md">
                Delete
              </button>
              <button className="font-poppins text-left px-2 py-1 text-white text-opacity-90 bg-[#4F1DDE] rounded-md">
                Edit
              </button>
            </div>
            <p className="font-poppins font-light text-sm pt-2 text-[#666666]">
              Mar 3, 2020
            </p>
            <div className="absolute bottom-0 right-0 mb-2 mr-3 inline-flex items-center justify-center w-12 h-12 overflow-hidden bg-gray-100 rounded-full">
              <span className="font-medium text-gray-600">A</span>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}

export default Todo;
